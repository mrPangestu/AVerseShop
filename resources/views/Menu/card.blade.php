...
<div class="card mx-3 p-0" data-product-id="{{ $product->id }}">
    <div class="card-background" style="background-image: url('{{ asset($product->image) }}');" class="card-img-top" height="170px" alt="{{ $product->product_name }}" 
        data-bs-toggle="modal" data-bs-target="#myModal"
        data-name="{{ $product->product_name }}"
        data-price="{{ number_format($product->price, 0, ',', '.') }}"
        data-stock="{{ $product->stock }}"
        data-description="{{ $product->description }}"
        data-image="{{ asset($product->image) }}"></div>
